...
- **API search:** `GET /v1.0/birds?search=<term>`. Two modes: (1) plain integer → exact `serialNumber` match only, no other conditions, min length 1; (2) text (min 3 chars) → word-boundary prefix regex (`\b<query>`, case-insensitive) on `name` and `scientificName`, plus taxonomic prefix resolution against cached lists of distinct `order`, `family`, and `commonGroup` values (1h NodeCache TTL) — matching values are added as `$in` clauses in the same `$or`. `group`, `family`, `order` query params remain as standalone exact-match filters. The list endpoint returns `{ id, serialNumber, name, scientificName, commonGroup }` — no images (meta join skipped for performance). Filter logic lives in `buildSearchFilterFromLists()` (exported pure function) — test it directly without mocking.
...
- **BOTD loading:** total bird count is cached in `localStorage` under `featherbase-bird-total` so `fetchBirdOfTheDay` can fire in parallel with `fetchBirds` on repeat visits (no sequential wait). On first visit the fetch is sequential but a `.botd-skeleton` placeholder (shimmer + content bones matching card dimensions) reserves the space and prevents layout shift. Skeleton is hidden when a group filter is active. `botdStarted` flag (plain `let`, not reactive) prevents the BOTD fetch from firing twice if the cache path and the birds-list path both try to start it.
...
- **Offline indicator:** `web/src/composables/networkStatus.ts` — `useNetworkStatus()` returns reactive `isOffline`. Primary signal: SW posts `NETWORK_OFFLINE` / `NETWORK_ONLINE` messages to all window clients when real fetch failures/recoveries occur (module-level `swIsOffline` flag in sw.js prevents message spam — only fires on state change). Secondary signals: browser `offline` event (fast, fires before any fetch) and `online` event + `/_health` probe (confirms real connectivity before clearing the banner). Banner is a sticky red strip at the top of the viewport (`.offline-banner` in `base.css`), slides in/out via Vue `<Transition>`, uses `--color-danger` token. `aria-live="polite"` for screen reader support.
